...
Adjacency_list dfs_degeneracy_order(int root, Adjacency_list adj,
                                    const vector<int> &core_numbers,
                                    bool reverse = false);

vector<int> path_l_to_bl(const Adjacency_list &adj, const vector<int> &deg,
                         int l) {
  int n_vertices = int(adj.size());
  std::queue<int> q;
...
Adjacency_list ilst(const Adjacency_list &adj) {
  ...
  //   bool has_d_leaf = false;

  if (is_hamiltonian_path || !has_degree_one)
    return dfs_tree;

  for (int l = 0; l < n_vertices; ++l) {
    if (deg[l] > 1)
      continue;
    if (!adj_t[l].contains(root))
      continue;
...
Adjacency_list dfs_degeneracy_order(int root, Adjacency_list adj,
  ...
  while (!edge_sources.empty()) {
    ...
    auto dest_itr =
        find_if_not(neighbors.cbegin() + neigh_offset, neighbors.cend(),
...
vector<int> get_core_numbers(const Adjacency_list &adj) {
  ...
  /// degrees (# of neighbors) of the vertices; this becomes the respective core
  /// numbers by the end of the algorithm
  vector<int> degrees(n);
  int max_degree{0};
  ranges::transform(adj, degrees.begin(),
                    [&max_degree](const Vertex_list &neighbors) {
  ...
  /// `degree_offsets[d]` == starting index (offset) of the subarray of d-degree
  /// vertices in the `vertices` array
  auto &degree_offsets = degree_freqs;
  partial_sum(degree_offsets.begin(), degree_offsets.end(),
              degree_offsets.begin());
  ...
  for (int i{0}; i < n; ++i) {
    ...
    for (auto u : adj[v]) {
      if (auto &du = degrees[u]; degrees[v] < du) {
        ...
        // the subarray with vertices having the same degree as the previous
        // degree of u now starts at one position to the right
        ++firsti;
      }
    }
  }
